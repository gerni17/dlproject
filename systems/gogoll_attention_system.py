...
class GogollAttentionSystem(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        # S --> S'' 
        attnMapS = toZeroThreshold(self.A_s(source_img), t=0.8)
        fgS = attnMapS * source_img
        bgS = (1 - attnMapS) * source_img
        genT = self.G_s2t(fgS) 
        fakeT = (attnMapS * genT) + bgS
        attnMapfakeT = toZeroThreshold(self.A_t(fakeT), t=0.8)
        ...
        # T --> T''
        attnMapT = toZeroThreshold(self.A_t(target_img), t=0.8)
        fgT = attnMapT * target_img
        bgT = (1 - attnMapT) * target_img
        genS = self.G_t2s(fgT) 
        fakeS = (attnMapT * genS) + bgT
        attnMapfakeS = toZeroThreshold(self.A_s(fakeS), t=0.8)
